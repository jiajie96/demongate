...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey:
		...
		if event.pressed and event.keycode == KEY_J and event.is_command_or_control_pressed():
			...
			GM.notify("Cheat: +%d Sins" % Config.CHEAT_SINS_AMOUNT, Color(1, 0.2, 0.8))
			get_viewport().set_input_as_handled()
			return
		if event.pressed and event.keycode == KEY_K and event.is_command_or_control_pressed():
			# Skip to target wave — clear current wave and jump
			GM.enemies.clear()
			...
			GM.notify("Cheat: Skipping to wave %d" % Config.CHEAT_SKIP_TO_WAVE, Color(1, 0.2, 0.8))
			get_viewport().set_input_as_handled()
			return
